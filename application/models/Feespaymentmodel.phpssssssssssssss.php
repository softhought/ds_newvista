...
/* component list by class and multiple month*/
	public function getFeesComponentListbyClassAndMonth($classid,$monthids,$acd_session_id,$accnt_year_id,$student_id){
		$where = array(		
			'fees_session.class_id' => $classid,
			 'fees_session.acdm_session_id' => $acd_session_id,
			 'fees_session.accnt_year_id' => $accnt_year_id,
		...
		SUM(CASE
		WHEN `payment_master`.`payment_id` IS NULL THEN `fees_session`.`amount`
		
		END
		) AS total_amount,
		GROUP_CONCAT( CASE
		WHEN `payment_master`.`payment_id` IS NULL THEN
		`fees_strucrure_month_dtl`.`month_id`
		END
		) AS month_id,
		GROUP_CONCAT( CASE
		WHEN `payment_master`.`payment_id` IS NOT NULL THEN
		`fees_strucrure_month_dtl`.`month_id`
		END
		) AS paidmonid,
		GROUP_CONCAT(
			CASE
			  WHEN `payment_master`.`payment_id` IS NOT NULL 
			  THEN month_master.month_code
			END
		  ) AS paidmoncode,
		  ...
		  `fees_session`.`amount`
				...
				->join('fees_session','fees_session.fees_id = fees_strucrure_month_dtl.fees_structure_id','INNER')
				->join('fees_structure','fees_structure.id = fees_session.fees_id','INNER')				
				->join('payment_details','payment_details.fees_component_id = fees_strucrure_month_dtl.fees_structure_id AND payment_details.month_id=fees_strucrure_month_dtl.month_id','LEFT')
				->join('payment_master','payment_master.payment_id=payment_details.payment_master_id AND payment_master.student_id='.$student_id,'LEFT')
				->join('month_master','payment_details.month_id = month_master.id','LEFT')
				->where($where)
				->where_in('fees_strucrure_month_dtl.month_id',$monthids)
				->order_by('fees_structure.id','ASC')
				->group_by('fees_session.fees_id')
				->get();
			// q();
			if($query->num_rows()> 0)
			{
...
/* component list by class and single month*/
	...
	public function getFeesComponentListbyClassMonth($classid,$monthid,$acd_session_id){
		$where = array(
						'fees_session.class_id' =>$classid,
						'fees_session.acdm_session_id' =>$acd_session_id,
	...
	public function getFeesComponentListSumbyClassMonth($classid,$monthid,$acd_session_id){
		$where = array(
						'fees_session.class_id' =>$classid,
						'fees_strucrure_month_dtl.month_id' =>$monthid
					);
	...
	public function getAlreadyPaidComponentBymonth($student_id,$acdm_session_id,$accnt_year_id,$school_id,$class_id,$month_id)
	{
		$component_id=[];
		$where=[
			"payment_master.student_id"=>$student_id,
			"payment_master.acdm_session_id"=>$acdm_session_id,
			"payment_master.accnt_year_id"=>$accnt_year_id,
			"payment_master.school_id"=>$school_id,
			"payment_master.class_id"=>$class_id
			
		];
		$query=$this->db->select('payment_details.fees_component_id')
						->from('payment_details')
						->join('payment_master','payment_details.payment_master_id = payment_master.payment_id ','INNER')
						->where($where)
						->where_in("payment_details.month_id",$month_id)
						->get();
			...
			if($query->num_rows()> 0)
			{
	          ...
	          foreach($query->result() as $rows)
				{
					$component_id[]=$rows->fees_component_id;
				}
				return $component_id;
	        }else {
				return "";
	...
	public function getFeesComponentListbyClassmonthWise($classid,$monthid,$acd_session_id,$component=null){
		$where = array(
						'fees_session.class_id' =>$classid,
						'fees_session.acdm_session_id' =>$acd_session_id,
						'fees_strucrure_month_dtl.month_id'=>$monthid
		...
		if (empty($component)) {
			$query = $this->db->select("
				...
				->join('fees_strucrure_month_dtl','fees_strucrure_month_dtl.fees_structure_id = fees_structure.id','INNER')
				->where($where)
				//->where_not_in('fees_session.fees_id', $component)
				//->where_in('fees_strucrure_month_dtl.month_id',$monthids)
				//->group_by('fees_structure.fees_desc')
				->get();
		} else {
