...
/* component list by class and single month*/
	...
	public function getPaidComponentIdForSelectedMonth($student_id,$school_id,$acd_session_id)
	{
		$data=[];
		$where=[
			"payment_master.student_id"=>$student_id,
			//"payment_master.class_id"=>$class_id,
			"payment_master.school_id"=>$school_id,
			"payment_master.acdm_session_id"=>$acd_session_id
		];
		$query = $this->db->select("*")
						  ->from('payment_master')	
						  ->join('payment_details',' payment_master.payment_id=payment_details.payment_master_id','INNER')
						  ->where($where)
						  ->get();
			if($query->num_rows()> 0)
			{
	...
	//  06-05-2019
	public function getPaidComponentId($student_id,$month_id,$class_id,$school_id,$acdm_session_id)
	{
		$data=[];
		$sql="SELECT * FROM payment_details
		INNER JOIN payment_master ON payment_details.payment_master_id=payment_master.payment_id AND payment_master.student_id=$student_id
		LEFT JOIN fees_session ON payment_details.fees_component_id=fees_session.fees_id
		LEFT JOIN fees_structure ON payment_details.fees_component_id=fees_structure.id
		LEFT JOIN month_master ON payment_details.month_id=month_master.id
		WHERE payment_details.month_id=$month_id
		AND fees_session.class_id=$class_id
		AND fees_session.school_id=$school_id
		and fees_structure.is_yearly_once='N'
		and fees_structure.is_once_in_life_time='N'	
		AND fees_session.acdm_session_id=$acdm_session_id";
		...
		if($query->num_rows()> 0)
		{
            foreach ($query->result() as $rows)
			{
				 $data[]=[
					...
					"fees_desc"=>$rows->fees_desc,
					"amount"=>$rows->amount,
					"month_id"=>$rows->month_id,
					"month_code"=>$rows->month_code,
				];
			}
			return $data;
        }else{
	...
	public function getAllComponentId($month_id,$class_id,$school_id,$acdm_session_id)
	{
		$data=[];
		$sql="SELECT fees_session.fees_id,fees_structure.fees_desc,fees_session.amount AS amountt,fees_strucrure_month_dtl.month_id,month_master.month_code FROM fees_session 
		INNER JOIN fees_structure ON fees_session.fees_id=fees_structure.id
		INNER JOIN fees_strucrure_month_dtl ON fees_session.fees_id=fees_strucrure_month_dtl.fees_structure_id
		INNER JOIN month_master ON fees_strucrure_month_dtl.month_id=month_master.id
		WHERE fees_strucrure_month_dtl.month_id=$month_id
		AND fees_session.class_id=$class_id
		AND fees_session.school_id=$school_id
		and fees_structure.is_yearly_once='N'
		and fees_structure.is_once_in_life_time='N'
	...
	public function getOnceInAyearComponentId($class_id,$school_id,$acdm_session_id)
	{
		...
		INNER JOIN fees_strucrure_month_dtl ON fees_session.fees_id=fees_strucrure_month_dtl.fees_structure_id
		INNER JOIN month_master ON fees_strucrure_month_dtl.month_id=month_master.id
		WHERE fees_session.class_id=$class_id 
		AND fees_session.school_id=$school_id
		and fees_structure.is_yearly_once='Y'
		and fees_structure.is_once_in_life_time='N'
		AND fees_session.acdm_session_id=$acdm_session_id
		GROUP BY fees_structure.id";
	...
	public function getOnceInAyearPaidComponentId($student_id,$class_id,$school_id,$acdm_session_id)
	{
		...
		LEFT JOIN fees_session ON payment_details.fees_component_id = fees_session.fees_id 
		LEFT JOIN fees_structure ON payment_details.fees_component_id = fees_structure.id 
		INNER JOIN month_master ON payment_details.month_id=month_master.id
		WHERE fees_session.class_id=$class_id 
		AND fees_session.school_id=$school_id
		and fees_structure.is_yearly_once='Y'
		and fees_structure.is_once_in_life_time='N'
	...
	public function getOnceInLifeTimeComponentId($school_id)
	{
		...
		INNER JOIN fees_strucrure_month_dtl ON fees_session.fees_id=fees_strucrure_month_dtl.fees_structure_id
		INNER JOIN month_master ON fees_strucrure_month_dtl.month_id=month_master.id
		WHERE fees_session.school_id=$school_id
		and fees_structure.is_yearly_once='N'
		and fees_structure.is_once_in_life_time='Y'		
		GROUP BY fees_structure.id";
	...
	public function getOnceInLifeTimePaidComponentId($student_id,$school_id)
	{
