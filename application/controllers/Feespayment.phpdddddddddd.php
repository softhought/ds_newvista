...
public function getStudent()
	...
	public function getPaymentComponentList()
	{
		...
		if($this->session->userdata('user_data'))
		{
			...
			$result['fessComponentData']=$this->feespaymentmodel->getFeesComponentListbyClassAndMonth($classid,$monthids,$session['acd_session_id'],$session['accnt_year_id'],$studentid);
			$result['checkAnyPaymentDoneAgainstThisStudent']=$this->commondatamodel->checkExistanceData('payment_master',array('student_id'=>$studentid));


			$AlreadyPaidComponent=$this->feespaymentmodel->getAlreadyPaidComponentBymonth($studentid,$session['acd_session_id'],$session['accnt_year_id'],$session['school_id'],$classid,$monthids);//paid component

			$ComponentListbyClassmonthWise=$this->feespaymentmodel->getFeesComponentListbyClassmonthWise($classid,$monthid,$acd_session_id,$component=$AlreadyPaidComponent);

		pre($ComponentListbyClassmonthWise);
...
public function getPaymentComponentListPaymentEdit()
	...
	public function payment_action()
	{
		...
		if($this->session->userdata('user_data'))
		{
			...
			$componentJsonData= $this->input->post('componentData');
			$fessComponentData=json_decode($componentJsonData);

			// pre($fessComponentData);
			// $monthid=explode(',',$monthids);
			// $month="";
			// $amnt=0;
			// $newArr=[];
			// $newArr1=[];
			...
			// for ($j=0; $j <sizeof($monthid) ; $j++) { 
			// 	for ($i=0; $i <sizeof($fessComponentData) ; $i++) { 
			// 		if ($monthid[$j]==$fessComponentData[$i]->month) {
			// 			array_push($newArr,$fessComponentData[$i]);
			// 		}
			// 	}
			// 	//echo "month -".$fessComponentData[$i]->month." amount -".$fessComponentData[$i]->amount."<br>";
				
			// 		foreach ($newArr as $item) {
			// 			$sum += $item->amount;
			// 		}
			// 		echo "month_id:".$monthid[$j]." SumAmount:".$sum;
			// 		$sum=0;
			// 	array_push($newArr1,$newArr);
			// 	$newArr=[];
			// }
			// pre($newArr1);
				...
				if($paymentID>0 && $mode=="EDIT")
				{
					...
					*/
					...
					foreach ($monthids_array as $key => $value) {
						$monthid=$value;
						$fessComponentListData=$this->feespaymentmodel->getFeesComponentListbyClassMonth($classid,$value,$acd_session_id);
						$fessComsumData=$this->feespaymentmodel->getFeesComponentListSumbyClassMonth($classid,$value,$acd_session_id);
